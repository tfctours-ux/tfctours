...
export default async function HotelBookingPage({
  ...
  const tPage = await getTranslations({
    locale,
    namespace: "servicePages.hotel-booking",
  });
  ...
  return (
    <>
      <FAQJsonLd locale={locale} items={SERVICE_FAQ_MAP["hotel-booking"]} />
      <ServiceJsonLd
        locale={locale}
        slug="hotel-booking"
        name={t("title")}
      ...
      <BreadcrumbJsonLd
        items={[
          ...
          {
            name: t("title"),
            path: localizePath(locale, "/services/hotel-booking"),
          },
        ]}
      />
      <Breadcrumb />
      ...
      <section className="mx-auto max-w-5xl px-6 py-16 border-t border-border mt-12">
        ...
        <Accordion
          items={SERVICE_FAQ_MAP["hotel-booking"].map((item) => ({
