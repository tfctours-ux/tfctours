...
export async function generateMetadata({
  ...
  return buildServiceMetadata(params, "ticket-booking");
}
...
export default async function TicketBookingPage({
  ...
  const tPage = await getTranslations({
    locale,
    namespace: "servicePages.ticket-booking",
  });
  ...
  return (
    <>
      <FAQJsonLd locale={locale} items={SERVICE_FAQ_MAP["ticket-booking"]} />
      <ServiceJsonLd
        locale={locale}
        slug="ticket-booking"
        name={t("title")}
      ...
      <BreadcrumbJsonLd
        items={[
          ...
          {
            name: t("title"),
            path: localizePath(locale, "/services/ticket-booking"),
          },
        ]}
      />
      <Breadcrumb />
      <ServiceDetailPage locale={locale} slug="ticket-booking" />
      <section className="mx-auto max-w-5xl px-6 py-16 border-t border-border mt-12">
        ...
        <Accordion
          items={SERVICE_FAQ_MAP["ticket-booking"].map((item) => ({
