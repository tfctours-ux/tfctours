...
export async function generateMetadata({
  ...
  return buildServiceMetadata(params, "tour-packages");
}
...
export default async function TourPackagesPage({
  ...
  const tPage = await getTranslations({
    locale,
    namespace: "servicePages.tour-packages",
  });
  ...
  return (
    <>
      <FAQJsonLd locale={locale} items={SERVICE_FAQ_MAP["tour-packages"]} />
      <ServiceJsonLd
        locale={locale}
        slug="tour-packages"
        name={t("title")}
      ...
      <BreadcrumbJsonLd
        items={[
          ...
          {
            name: t("title"),
            path: localizePath(locale, "/services/tour-packages"),
          },
        ]}
      />
      <Breadcrumb />
      <ServiceDetailPage locale={locale} slug="tour-packages" />
      <section className="mx-auto max-w-5xl px-6 py-16 border-t border-border mt-12">
        ...
        <Accordion
          items={SERVICE_FAQ_MAP["tour-packages"].map((item) => ({
