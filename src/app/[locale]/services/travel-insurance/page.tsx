...
export default async function TravelInsurancePage({
  ...
  const tPage = await getTranslations({
    locale,
    namespace: "servicePages.travel-insurance",
  });
  ...
  return (
    <>
      <FAQJsonLd locale={locale} items={SERVICE_FAQ_MAP["travel-insurance"]} />
      <ServiceJsonLd
        locale={locale}
      ...
      <BreadcrumbJsonLd
        items={[
          ...
          {
            name: t("title"),
            path: localizePath(locale, "/services/travel-insurance"),
          },
        ]}
      />
      <Breadcrumb />
      ...
      <section className="mx-auto max-w-5xl px-6 py-16 border-t border-border mt-12">
        ...
        <Accordion
          items={SERVICE_FAQ_MAP["travel-insurance"].map((item) => ({
