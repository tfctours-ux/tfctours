...
export function Breadcrumb() {
  ...
  return (
    <nav
      aria-label={t("aria")}
      className="mx-auto flex w-full max-w-7xl flex-wrap items-center gap-2 px-6 pt-8 text-xs uppercase tracking-[0.14em] text-foreground-muted"
    >
        ...
        return (
          <span key={item.href} className="inline-flex items-center gap-2">
            {index === 0 ? (
              ...
              <ChevronRight className="h-4 w-4 text-foreground-subtle rtl:rotate-180" />
            )}
            {isLast ? (
