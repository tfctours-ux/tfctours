// src/components/shared/ServiceDetailPage.tsx
      ...
      <PageHero
        ...
        aside={
          <div className="space-y-3">
            <Badge className="border-accent-foreground/20 bg-accent-foreground/15 text-accent-foreground backdrop-blur-sm">
              {catalogT(`${slug}.tag`)}
            </Badge>
            <div className="grid gap-2">
              {highlights.map((item) => (
                <div
                  key={item}
                  className="flex items-start gap-3 rounded-2xl border border-accent-foreground/15 bg-scrim/40 px-4 py-3 text-sm leading-6 text-accent-foreground/90 backdrop-blur-md"
                >
      ...
      <div className="mx-auto mt-14 max-w-7xl px-6">
        <div className="grid gap-8 lg:grid-cols-[1fr_1fr]">
          ...
          <div className="rounded-[2rem] border border-border bg-surface-elevated p-6 shadow-brand md:p-8">
            ...
            <ol className="mt-6 space-y-4">
              {process.map((step, index) => (
                ...
                >
                  <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-accent text-xs font-black text-accent-foreground">
                    {index + 1}
                  </span>
          ...
          <div className="space-y-8">
            ...
            {/* CTA dark card */}
            <div className="overflow-hidden rounded-[2rem] bg-background shadow-glow">
              ...
              <div className="px-6 pb-8 pt-2 text-foreground md:px-8">
                ...
                <div className="mt-6 flex flex-wrap gap-3">
                  ...
                  <Button
                    ...
                    variant="outline"
                    className="border-foreground/15 bg-foreground/10 text-foreground hover:border-foreground hover:bg-foreground hover:text-background"
                  >
                ...
                {SERVICES.filter((item) => item.slug !== slug)
                  ...
                  .map((item) => (
                    <Link
                      ...
                      className="group flex items-center justify-between rounded-2xl border border-border bg-surface px-4 py-3.5 text-sm font-medium text-foreground transition hover:border-accent/30 hover:bg-accent/10 hover:text-accent"
                    >
